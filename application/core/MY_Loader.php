...
class MY_Loader extends CI_Loader {
	...
	function css(){
		...
		foreach($css_files as $css_file){
			...
			if(!$is_external)
				if(!file_exists('assets/'.$css_file))
					show_error("Cannot locate stylesheet file: {$css_file}.");

			$css_file = $is_external == FALSE ? base_url('assets/') . $css_file : $css_file;
	...
	function less(){
		$less_files = func_get_args();

		foreach($less_files as $less_file){
			$less_file = substr($less_file,0,1) == '/' ? substr($less_file,1) : $less_file;

			$is_external = false;
			if(is_bool($less_file))
				continue;

			$is_external = preg_match("/^https?:\/\//", trim($less_file)) > 0 ? true : false;

			if(!$is_external)
				if(!file_exists('assets/'.$less_file))
					show_error("Cannot locate stylesheet file: {$less_file}.");

			$less_file = $is_external == FALSE ? base_url('assets/') . $less_file : $less_file;

			if(!in_array($less_file, $this->_less))
				$this->_less[] = $less_file;
		}
		return;
	}
	...
	function js(){
		...
		foreach($script_files as $script_file){
			...
			if(!$is_external)
				if(!file_exists('assets/'.$script_file))
					show_error("Cannot locate javascript file: {$script_file}.");

			$script_file = $is_external == FALSE ?  base_url('assets/') . $script_file : $script_file;
	...
	function get_cached_less_files(){
		return $this->_cached_less;
	}
